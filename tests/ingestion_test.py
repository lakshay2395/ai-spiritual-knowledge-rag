...
class TestIngestion:
    ...
    @patch("builtins.open")
    def test_parse_bible_mocked(
        self, mock_open, mock_listdir, mock_exists, mock_makedirs
    ):
...
"""
        ...
        mock_open.side_effect = [StringIO(bible_content), output_mock]
    ...
    @patch("builtins.open")
    def test_parse_gita_mocked(
        self, mock_open, mock_listdir, mock_exists, mock_makedirs
    ):
        mock_exists.side_effect = lambda p: True
        # listdir for raw_dir
        # listdir for folder_path
        mock_listdir.side_effect = [["1"], ["1.md"]]  # chapters  # verses in chapter 1
...
..."""
        ...
        output_mock = MagicMock()
        mock_open.side_effect = [StringIO(gita_content), output_mock]
        output_mock.__enter__.return_value = output_mock
