...
class HybridSearchTester:
    ...
    def load_indices(self, vector_dir: str, keyword_dir: str):
        ...
        for file in os.listdir(vector_dir):
            if file.endswith(".faiss"):
                ...
                self.vector_indices[name] = faiss.read_index(
                    os.path.join(vector_dir, file)
                )
    ...
    def semantic_search(
        self, query: str, source: str, top_k: int = 3
    ) -> List[Dict[str, Any]]:
        """
    ...
    def display_results(self, results: List[Dict[str, Any]], source: str):
        ...
        for i, res in enumerate(results, 1):
            ...
            snippet = (
                res["text"][:150].replace("\n", " ") + "..."
                if len(res["text"]) > 150
                else res["text"]
            )

            print(
                f"{i}. [{citation}] | Score: {res['score']:.4f} | Method: {res['method']}"
            )
...
if __name__ == "__main__":
    ...
    try:
        ...
        fused_results = retriever.get_top_k(
            "What does the text say about eternal life?", top_k=5
        )

        for i, res in enumerate(fused_results, 1):
            print(
                f"{i}. [{res['citation']}] (RRF: {res['rrf_score']:.5f}, Re-rank: {res['rerank_score']:.4f})"
            )
