...
def main():
    ...
    report_file = st.sidebar.selectbox(
        "Select Report File",
        ["data/evaluation/ragas_report_optimized.csv", "data/evaluation/ragas_report.csv"]
    )
    ...
    with chart_col1:
        ...
        fig_dist = px.histogram(df, x=selected_metric, nbins=20, 
    ...
    with chart_col2:
        ...
        fig_scatter = px.scatter(df, x=metric_x, y=metric_y, hover_data=["user_input"],
    ...
    fig_bar = px.bar(avg_df, x="Metric", y="Average Score", color="Metric",
    ...
    if search_query:
        df_display = df[df["user_input"].str.contains(search_query, case=False, na=False)]
    else:
        df_display = df
